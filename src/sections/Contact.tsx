...
export default function Contact() {
  ...
  return (
    <SectionWrapper id="contact" className="py-24 max-w-3xl mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white/80 dark:bg-zinc-900/80 rounded-2xl shadow-lg p-5 sm:p-8 flex flex-col gap-4">
        ...
        <button type="submit" disabled={isSubmitting} className="w-full sm:w-auto rounded-full px-6 sm:px-8 py-3 bg-green-600 text-white font-semibold shadow-lg hover:bg-green-700 transition-all mt-2 disabled:opacity-60">
      ...
      <div className="flex flex-col items-center mt-8 gap-2">
        ...
        <div className="text-zinc-700 dark:text-zinc-200 text-sm text-center break-all sm:break-normal">
          <span>{SITE.email}</span>
          <span className="hidden sm:inline"> &bull; {SITE.phone}</span>
          <span className="block sm:hidden">{SITE.phone}</span>
        </div>
      </div>
